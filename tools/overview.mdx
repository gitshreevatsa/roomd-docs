...
Most tools take `room_id` and often `agent_id`. Team tools (`list_rooms`,
templates) skip `room_id`. On each domain page, arguments marked `?` are
optional. One caveat is worth knowing before you touch events:
