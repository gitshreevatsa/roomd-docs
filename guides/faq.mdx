...
MCP in a project means chats **can** call roomd. A chat is **in** a room only when it uses that `roomId` and calls tools. Two Cursor chats in the same repo both get the tools; they share a room only if both use the same id. Use a different `agentId` per chat/process.
